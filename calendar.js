 ...
 */
function convertGregorianToNatural(date) {
...
function updateSidebar(date) {
    ...
    if (monthEl)
        monthEl.textContent = natDate.monthName;
    if (yearEl)
        yearEl.textContent = `Year ${natDate.year}`;
    if (dayEl)
        dayEl.textContent = natDate.day === 0 ? '∞' : String(natDate.day);
    if (gregEl) {
        const options = {
            month: 'long',
            day: 'numeric',
    ...
    if (phaseEl)
        phaseEl.textContent = moon.phaseName;
    if (illumEl)
        illumEl.textContent = `${Math.round(moon.illumination * 100)}% Illumination`;
    if (moonVisual)
        renderMoonVisual(moonVisual, moon);
 ...
 */
function getEsotericInsight(moon) {
 ...
 */
function renderMoonVisual(container, moonData) {
 ...
 */
function renderAeternaCard(gregorianYear, naturalYear, grid, label) {
    ...
    aeternaCell.onclick = () => {
        ...
        const dateInput = document.getElementById('gregorianInput');
        if (dateInput)
 ...
 */
function renderYearGrid(naturalYear) {
    ...
    if (titleEl)
        titleEl.textContent = `Natural Year ${naturalYear}`;
